...
$$
## goodness of fit
- Under the assumption that a model is correct, the deviance is distributed $\chi^2_{n-q-1}$; a low p-value under this distribution indicates a poor-fit.
...
&= \frac{y_i - n_i\hat{p_i}}{n_i \hat{p_i} * (1 - \hat{p_i})}
\end{aligned}
$$
...
- log likelihood
$$
\sum_i y_i \cdot log(p_i) + (m_i - y_i) \cdot log(1 - p_i)
$$
...
- confusion matrix is just the matrix of (true response, predicted response) counts
- accuracy of the model is just the number of correct cases divided by the total number of cases in the matrix
- but confusion matrix provides a breakdown of what constitutes our accuracy. false positive / false negatives may be weighted differently than true positive / true negative.
- sensitivity: what fraction of true positives were identified correctly?
- specificity: what fraction of true negatives were identified correctly?
...
## overdispersion
- overdispersion is when the observed variance is far in excess of the expected variance

## f statistics
- Since $\sigma$ is estimated from data, we must use an f-statistic to determine the significance of extra predictors in the presence of overdispersion
